...
class Test(unittest.TestCase):
    def test(self):
        ...
        print(length)
        value = 'kruthigeneral@gmail.com'
        for i in range(1,length+1):
            rows = driver.find_element_by_xpath("//*[@id='dt_basic']/tbody/tr["+str(i)+"]/td[3]").text
            users.append(rows)
            print(rows)
        if value in users:
            manage_user_obj.manage_user_search("kruthigeneral@gmail.com")
            time.sleep(3)
...
# Validation: To check whether added user is present in list or not ; If present " User Added Successfully will be printed, If not Exception will be raised
        ...
        for i in range(1,length):
            deletedrows = driver.find_element_by_xpath("//*[@id='dt_basic']/tbody/tr["+str(i)+"]/td[3]").text
            users.append(deletedrows)
            print(deletedrows)
            self.assertNotIn(value,deletedrows)   #compares the value present in value with the user list
